...
export default function ExerciseInformation({
	...
	const exercise = exercises.find((e) => e.name === chosenExercise.name);

	useEffect(() => {
		if (exercise?.setsCompleted === exercise?.sets) {
			setExercises((prevExercises) => {
				return prevExercises.map((exercise) =>
					exercise.name === chosenExercise.name
						? {
								...exercise,
								["done"]: true,
						  }
		...
		return (
			<div className="flex flex-col gap-4" key={chosenExercise.name}>
				<span className="text-2xl font-bold">
					{exercise && exercise.setsCompleted}/{chosenExercise.sets}{" "}
					SETS
				</span>
				{setDivs}
			</div>
	...
	return chosenExercise ? (
		<div
			className="h-full pl-5 flex flex-col gap-8"
